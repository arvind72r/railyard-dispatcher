...
(function (root) {
  ...
  RY.steamGeo = function (cls, len) {
    ...
    if (cls === 'wg') {
      g = { cls: cls, hl: hl, nose: hl - 3, smokeRear: hl - 16, boilerRear: 8, cabRear: -14, tenderFront: -18,
            chimney: hl - 9, dome: 30, valve: 14, rB: 11.5, zB: 31, bullet: false,
      ...
      g = { cls: 'wp', hl: hl, nose: hl - 2, smokeRear: hl - 17, boilerRear: 12, cabRear: -10, tenderFront: -14,
            chimney: hl - 10, dome: 34, valve: 16, rB: 12.5, zB: 31, bullet: true,
  ...
  RY.drawSteamLoco = function (ctx, tr, vh) {
    ...
    ctx.fillStyle = 'rgba(200,204,210,.18)';
    ctx.fillRect(g.cabRear, -16, hl - g.cabRear - 2, 1.4); ctx.fillRect(g.cabRear, 14.6, hl - g.cabRear - 2, 1.4);
    ...
    sg.addColorStop(0, '#060606'); sg.addColorStop(0.5, '#3a3b3d'); sg.addColorStop(1, '#060606');
    ctx.fillStyle = sg;
    ctx.beginPath();
    ctx.moveTo(g.smokeRear, -g.rB);
    if (g.bullet) {
      ctx.lineTo(g.nose - 9, -g.rB);
      ctx.quadraticCurveTo(g.nose + 1, -g.rB * 0.8, g.nose + 1, 0);
      ctx.quadraticCurveTo(g.nose + 1, g.rB * 0.8, g.nose - 9, g.rB);
    } else {
      ctx.lineTo(g.nose, -g.rB); ctx.lineTo(g.nose, g.rB);
    }
    ctx.lineTo(g.smokeRear, g.rB); ctx.closePath(); ctx.fill();
    if (g.bullet) {
      ctx.strokeStyle = '#d7d9dc'; ctx.lineWidth = 1.3;
      ctx.beginPath(); ctx.arc(g.nose - 7, 0, g.rB * 0.72, -1.1, 1.1); ctx.stroke();
    }
    ...
    ctx.fillStyle = '#9ea3a8'; ctx.fillRect(g.nose - 13, -2, 5, 4);
    ...
    ctx.fillStyle = '#1d2229'; ctx.fillRect(hl, -8, 4, 4); ctx.fillRect(hl, 4, 4, 4);
